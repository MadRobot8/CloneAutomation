...
echo -e "$(date +"%d-%m-%Y %H:%M:%S"):${HOST_NAME}:${current_task_id}:DB EXTRACT: Logfile at  ${HOST_NAME}: ${dbextractlog}" | tee "${dbextractlog}"
...
rm -f /tmp/temp${dblower,,}spfile01.tmp /tmp/temp${dblower,,}control01.tmp  >/dev/null 2>&1
...
sqlplus -s "${APPSUSER}"@"${dbupper}" << EOF
set head off
set feed off
set line 999
...
sqlplus  / 'as sysdba' << EOF  >/dev/null
set echo on ;
spool ${log_dir}/bouncedb${trgdbname^^}.${startdate}
SHUTDOWN IMMEDIATE;
STARTUP
spool off
exit
...
if (( current_task_id <  1000  ||  current_task_id > 1800  )) ; then
  echo -e "$(date +"%d-%m-%Y %H:%M:%S"):${HOST_NAME}: DB TASK ID CHECK : TASK ID is out of range for Database script execution."
  exit 1
fi
