...
if [[ -z "${workappspass}" ]] ; then
  echo -e "$(date +"%d-%m-%Y %H:%M:%S"):${HOST_NAME}: ERROR: Source Environment password is not provided.\n"
  exit 1
fi
...
echo -e "$(date +"%d-%m-%Y %H:%M:%S"):${HOST_NAME}:*                 Database recovery time : ${time_flag}               *"
echo -e "$(date +"%d-%m-%Y %H:%M:%S"):${HOST_NAME}:*                                                                     *"
echo -e "$(date +"%d-%m-%Y %H:%M:%S"):${HOST_NAME}:*                                                                     *"
echo -e "$(date +"%d-%m-%Y %H:%M:%S"):${HOST_NAME}:***********************************************************************"
echo -e "\n"
...
if [[ "${prepareapp_stage}" == "COMPLETED" ]]; then
  echo -e "$(date +"%d-%m-%Y %H:%M:%S"):${HOST_NAME}:${current_task_id}:PREPARE APP: PREPARE Application for restore stage is already completed. Moving on."
elif [[ "${current_task_id}" == 50 ]] && [[ "${session_type}" == "NEW" ]] ; then
    #******************************************************************************************************##
    ...
    ssh -q "${trgappsosuser}"@"${trgapphost}"."${labdomain}" " nohup sh ${inst_bin}/clonedbapp.sh   & "
...
if [[ "${db_stage}" == "COMPLETED" ]]; then
  ...
  ssh -q "${trgdbosuser}"@"${trgdbhost}"."${labdomain}" " nohup sh ${inst_bin}/clonedbdb.sh  ${workappspass} & "
