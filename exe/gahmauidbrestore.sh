#!/bin/bash
#******************************************************************************************************
# 	$Header 1.2 2022/07/28 dikumar $
#  Purpose  : Script to Check and restore database from specified date.
#
#  SYNTAX   : sh <instance name>restore.sh
#             sh orasuprestore.sh
#
#  Author   : Dinesh Kumar
#
...
#******************************************************************************************************##
#  **********  D A T A B A S E - R E S T O R E - S C R I P T **********
#******************************************************************************************************##
...
${ECHO} "     *********************************************************************************"
${ECHO} "    "
${ECHO} "                      THIS IS START OF THIS ${dbupper} RESTORE SESSION.   "
${ECHO} "    "
${ECHO} "     *********************************************************************************"
sleep 4
...
if [ ! -f "${envfile}" ];  then
    ${ECHO} "ERROR: Target Environment profile ${envfile} file not found.\n"
    exit 1;
else
...
source "${lib_home}"/fundbpostrestore.sh
source "${lib_home}"/os_check_dir.sh
source "${lib_home}"/dbready.sh
source "${lib_home}"/fungahrman.sh
...
if [ "${session_task}" = 0 ]; then
  if [ "${session_stage}" = "PREPARE_DB" ]; then
      # First task for PREPARE_DB stage
      session_task=1500
  elif [ "${session_stage}" = "RESTORE_DB" ]; then
      # First task for PREPARE_DB stage
      session_task=1600
  elif [ "${session_stage}" = "POSTRESTORE_DB" ]; then
      # First task for PREPARE_DB stage
      session_task=1800
  fi
elif [ "${session_task}" -gt 3000 ]; then
