#!/bin/bash
#******************************************************************************************************#
#  Purpose: pullappsbackup.sh will check remote backup location and pull tar backup for clone tasks. 
#
#  SYNTAX : sh pullappsbackup.sh    
#
...
if [  -f ${trgappsbkploc}/newrunfs.backup ] && [  ! -f ${trgappsbkploc}/adhoc.request ] ; then
	 ${ECHO} "OEM: GAHPRD : Another session of this script is already running. Exiting !!\n"  | tee -a  "${logf}"
   exit 0
elif [  -f ${trgappsbkploc}/newrunfs.backup ] && [  -f ${trgappsbkploc}/adhoc.request ] ; then
  ${ECHO} "OEM: GAHPRD : Another session of this script is already running. "  | tee -a  "${logf}"
  ${ECHO} "OEM: GAHPRD : There is a request for ad-hoc backup, current backup will be used for ad-hoc purpose. "  | tee -a  "${logf}"
...
if [ ! -f "${trgappsbkploc}"/"${appsbkfile}" ]; then
