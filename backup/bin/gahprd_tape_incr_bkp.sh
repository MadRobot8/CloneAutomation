...
rman TARGET sys/YfXqM0pNs079bp1b@GAHPRD1 CATALOG rmancat/h1ghland3r@RMANDB log $LOG_DIR/rman_incr_bkp_GAHPRD_"${DATE}".log << EOF
RUN
{
...
ERRCNT=$(grep -ic RMAN- $LOG_DIR/rman_incr_bkp_GAHPRD_"${DATE}".log)
if [ "$ERRCNT" = 0 ]
then
mailx -s "GAHPRD RMAN Incremental Backup Completed ...Successfully" $MAILID < $LOG_DIR/rman_incr_bkp_GAHPRD_"${DATE}".log
else
mailx -s "CRITICAL: GAHPRD RMAN Incremental Backup Completed with Errors. Please Review the Log" $MAILID1 < $LOG_DIR/rman_incr_bkp_GAHPRD_"${DATE}".log
fi

rm -f /dba/tmp/gahprdincrbkp.lock > /dev/null 2>&1
