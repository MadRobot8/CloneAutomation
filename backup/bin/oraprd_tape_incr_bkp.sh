#!/bin/bash
#******************************************************************************************************##
#  **********  D A T A B A S E - I N C R E M E N T A L - N E T B A C K U P - T A P E - B A C K U P - S C R I P T **********
#******************************************************************************************************##

if [ -f /dba/tmp/oraprdincrbkp.lock ]; then
  printf "ORAPRD Full/Hot Backup script is already running. Exiting this run!!";
  exit 1
else
  echo $$ > /dba/tmp/oraprdincrbkp.lock
fi
...
printf "ORAPRD Incremental Backup is Running....\n"
rman TARGET sys/Pm_K5Bru@ORACPRD2 CATALOG rmancat/h1ghland3r@RMANDB log $LOG_DIR/rman_incr_bkp_ORAPRD_"${DATE}".log << EOF
RUN
{
...
ERRCNT=$(grep -ic RMAN- $LOG_DIR/rman_incr_bkp_ORAPRD_"${DATE}".log)
if [ "$ERRCNT" = 0 ]
then
mailx -s "ORAPRD RMAN Incremental Backup Completed ...Successfully" $MAILID < $LOG_DIR/rman_incr_bkp_ORAPRD_"${DATE}".log
else
mailx -s "CRITICAL: ORAPRD RMAN Incremental Backup Completed with Errors. Please Review the Log" $MAILID1 < $LOG_DIR/rman_incr_bkp_ORAPRD_"${DATE}".log
fi

rm -f /dba/tmp/oraprdincrbkp.lock > /dev/null 2>&1

exit
#******************************************************************************************************##
#  **********  D A T A B A S E - I N C R E M E N T A L - N E T B A C K U P - T A P E - B A C K U P - S C R I P T - E N D **********
#******************************************************************************************************##
